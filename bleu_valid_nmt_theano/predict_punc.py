...
class Translator(object):
    ...
    def __init__(self, settings):
        ...
        # load model options
        self._load_model_options(settings.options)
        # load and invert dictionaries
        self._build_dictionaries(settings.source_dic, settings.target_dic)
        # set up queues
        self._init_queues()
    ...
    def _load_theano(self):
        ...
        from predict import (build_sampler, gen_sample)
    ...
    def _translate(self, process_id, input_item, trng, fs_init, fs_next, gen_sample):
        ...
        sample, score, word_probs = self._sample(input_item, trng, fs_init, fs_next, gen_sample)

        if nbest is True:
            output_item = sample, score, word_probs
        else:
