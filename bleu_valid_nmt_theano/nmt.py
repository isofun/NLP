...
from batch_beam_search import _mask_probs, _tensor_gather_helper, process_beam_results
...
def _beam_search_step(time_, logits, prev_states, beam_scores, beam_mask, batch_size, beam_size, options):
    """
    ...
    vocab_size = options['n_words']

    # shape is [batch_size, beam_size, vocab_size]
    next_scores = - tensor.nnet.logsoftmax(logits)
    next_scores = next_scores.reshape([batch_size, beam_size, vocab_size])

    next_scores = _mask_probs(scores=next_scores, beam_mask=beam_mask)
    ...
    next_beam_scores = ifelse(tensor.eq(time_, 0.0), next_beam_scores[:,:vocab_size], next_beam_scores)


    # [batch_size, beam_size]
    next_beam_scores, word_indices = tensor.topk_and_argtopk(- next_beam_scores, axis=-1, sorted=False, kth=beam_size)
    next_beam_scores = - next_beam_scores

    # Final shape is [batch_size, beam_size]
    next_beam_ids, next_word_ids = tensor.divmod(word_indices, vocab_size)
    # shape is [batch_size, beam_size]
    next_beam_mask = _tensor_gather_helper(gather_indices=next_beam_ids,
    ...
    next_beam_mask_ = tensor.switch(tensor.eq(next_word_ids, numpy.int64(0)), 0.0, 1.0)
    next_beam_mask = next_beam_mask * next_beam_mask_

    next_states = _tensor_gather_helper(gather_indices=next_beam_ids,
...
def build_beam_sampler(tparams, options, use_noise, trng, beam_size=5, return_alignment=False):
    dropout = dropout_constr(options, use_noise, trng, sampling=True)
    x_mask = tensor.matrix('x_mask', dtype=floatX)
    x, ctx = build_encoder(tparams, options, dropout, x_mask=x_mask, sampling=True)
    batch_size = ctx.shape[1]
    ctx_mean = (ctx * x_mask[:, :, None]).sum(0) / x_mask.sum(0)[:, None]
    init_state = get_layer_constr('ff')(tparams, ctx_mean, options, dropout,
                                        prefix='ff_state', activ='tanh')
    def _decode_step(time_, prev_word_ids, prev_states, beam_mask,
        ...
        prev_states_ = prev_states.reshape((1, batch_size * beam_size, options['dim']))
        beam_mask_ = beam_mask.flatten()

        logit, _, next_states = build_decoder(tparams, options,
                                              prev_input_, ctx, prev_states_,
                                              dropout, x_mask=ctx_mask, y_mask=beam_mask_,
                                              sampling=True)
        ...
        return [time_ + 1.0, next_word_ids, next_states, next_beam_mask, next_beam_ids, next_beam_scores], {}, \
               theano.scan_module.until(tensor.all(tensor.eq(next_beam_mask, 0.0)))
    init_time_ = tensor.constant(0.0, dtype='float32')
    init_word_ids = - tensor.ones(shape=(batch_size, beam_size), dtype='int64')
    init_states = tensor.tile(init_state[:,None,:], [1, beam_size, 1]).reshape([batch_size, beam_size, -1])
    init_beam_mask = tensor.ones(shape=[batch_size, beam_size], dtype='float32')
    init_beam_ids = tensor.zeros(shape=[batch_size, beam_size], dtype='int64')
    init_beam_scores = tensor.zeros(shape=[batch_size, beam_size], dtype='float32')

    ctx_ = tensor.tile(ctx[:,:,None,:], [1, 1, beam_size, 1]).reshape([ctx.shape[0],
                                                                       beam_size * batch_size,
                                                                       ctx.shape[-1]])
    ctx_mask_ = tensor.tile(x_mask[:, :, None], [1, 1, beam_size]).reshape([x_mask.shape[0],
                                                                            batch_size * beam_size])
    res, update= theano.scan(_decode_step,
                             outputs_info=[init_time_, init_word_ids, init_states,
                                           init_beam_mask, init_beam_ids, init_beam_scores],
                             non_sequences=[tparams['Wemb_dec'], ctx_mask_, ctx_],
                             strict=False,
                             n_steps=200
                             )
    final_word_ids = res[1] # [seq_len, batch_size, beam_size]

    final_beam_ids = res[4] # [seq_len, batch_size, beam_size]
    final_beam_scores = res[5][-1] # [batch_size, beam_size]

    f_batch_beam_search = theano.function(inputs=[x, x_mask],
                                          outputs=[final_word_ids, final_beam_ids, final_beam_scores],
                                          updates=update,
...
def train(dim_word=512,  # word vector dimensionality
    ...
    best_models= dict()
    popens = []
    training_progress.best_bleu = -1
    training_progress.bad_counter = 0
    training_progress.decode_bad_counter = 0
    ...
    training_progress.history_bleu = []
    ...
    if validFreq or sampleFreq:
        ...
        logging.info('Building batch beam sampler...')
        f_batch_beam_sampler = build_beam_sampler(tparams, model_options, use_noise, trng)
        logging.info('Done')
    if model_options['objective'] == 'MRT':
    ...
    # def process_former_bleu(bleu_validator, popens, bleu_model_tlist, best_p, best_opt_p):
    #     this_bleu = -1
    #     if len(popens) > 0:
    #         popen, this_p, this_opt_p, this_model, this_trans, this_uidx, this_eidx, this_valid = popens.pop()
    #         popen.wait()
    #         this_bleu = bleu_validator.test_bleu(this_trans)
    #         if this_bleu > training_progress.best_bleu:
    #             training_progress.best_bleu = this_bleu
    #             training_progress.best_uidx = this_uidx
    #             training_progress.best_eidx = this_eidx
    #             best_p = this_p
    #             best_opt_p = this_opt_p
    #             os.system('cp %s %s' % (this_model, saveto))
    #         # print 'fortest1', this_uidx, dict_best_params['uidx']
    #         if this_bleu > 0:
    #             bleu_model_tlist.append((this_bleu, this_model))
    #             # display BLEU info
    #             # print 'Epoch', this_eidx, 'Update', this_uidx, 'BLEU', this_bleu, 'Valid', this_valid, \
    #             #     ('\tBest BLEU %f (Epoch: %d, Update: %d)' % (training_progress.best_bleu,
    #             #                                                  training_progress.best_eidx,                                                                    training_progress.best_uidx))
    #             logging.info('Epoch %s Update %s BLEU %s Valid %s Best BLEU %f (Epoch: %d, Update: %d)' %
    #                          (this_eidx, this_uidx, this_bleu, this_valid,
    #                           training_progress.best_bleu,
    #                           training_progress.best_eidx,
    #                           training_progress.best_uidx))
    #             # remove old models
    #             best_bleu = training_progress.best_bleu
    #             newlist = []
    #             while len(bleu_model_tlist) > 0:
    #                 b, m = bleu_model_tlist.pop()
    #                 if (best_bleu - b) * 1. / best_bleu >= 0.1 :
    #                     if int(m.split('/')[-1].split('.')[1][4:]) % saveFreq != 0:
    #                         logging.info('Remove %s' % m)
    #                         os.system('rm %s' % m)
    #                         os.system('rm %s' % (m+'.gradinfo.npz'))
    #                         os.system('rm %s' % (m + '.progress.json'))
    #                 else:
    #                     newlist.append((b, m))
    #             bleu_model_tlist = newlist
    #             # print 'fortest2', this_uidx, dict_best_params['uidx']
    #     return bleu_validator, popens, bleu_model_tlist, best_p, best_opt_p, this_bleu
    ...
    for training_progress.eidx in xrange(training_progress.eidx, max_epochs):
        ...
        for x, y in train:
            ...
            if valid is not None and validFreq and numpy.mod(training_progress.uidx, validFreq) == 0:
                ...
                if valid_err > numpy.array(training_progress.history_errs).min():
                    ...
                    logging.info('Valid bad counter %s at %s' % (
                        training_progress.bad_counter,
                        training_progress.uidx))
            ...
            if valid is not None and decodeFreq and \
                            numpy.mod(training_progress.uidx, decodeFreq) == 0 and \
                            training_progress.uidx > decodeFreq:

                saveto_uidx = '{}.iter{}.npz'.format(
                    os.path.splitext(saveto)[0], training_progress.uidx)
                prefix = bleu_validator.tmp_dir + saveto.split('/')[-1]
                trans_saveto = "%s.trans.iter%d" % (prefix, training_progress.uidx)
                logging.info('Start decoding at iter %s ...' % training_progress.uidx)
                bleu_validator.batch_decode(f_batch_beam_sampler, trans_saveto, model_options)
                logging.info('Decoding finished')
                bleu_score = bleu_validator.test_bleu(trans_saveto)
                if len(best_models) < 10 or bleu_score > numpy.array(best_models.values()).min():
                    logging.info('Saving the model at iteration {}...'.format(training_progress.uidx))
                    former_p = unzip_from_theano(tparams, excluding_prefix='prior_')
                    former_opt_p = unzip_from_theano(optimizer_tparams, excluding_prefix='prior_')
                    save(params, optimizer_params, training_progress, saveto_uidx)
                    best_models[saveto_uidx] = bleu_score
                    kk = ''
                    for k, v in best_models.iteritems():
                        if v == numpy.array(best_models.values()).min() and \
                                v != saveto_uidx:
                            kk = k
                    if best_models.has_key(kk):
                        del best_models[kk]
                        logging.info('Remove model %s' % kk)
                        os.system('rm %s' % kk)
                        os.system('rm %s' % (kk+'.gradinfo.npz'))
                        os.system('rm %s' % (kk + '.progress.json'))
                    else:
                        logging.error('No model %s in best models list' % kk)
                    logging.info('Done')
                if len(best_models) > 0:
                    if bleu_score > numpy.array(best_models.values()).max():
                        training_progress.bad_counter = 0
                        training_progress.decode_bad_counter = 0
                    elif bleu_socre < numpy.array(best_models.values()).max():
                        training_progress.decode_bad_counter += 1
                        logging.info('Decoding bad counter %s at %s' % (
                            training_progress.decode_bad_counter, training_progress.uidx))
                if training_progress.decode_bad_counter > (patience * 1000):
